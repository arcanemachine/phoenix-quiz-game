defmodule QuizGameWeb.Quizzes.QuizLive.Stats do
  ...
  defp _user_detail(assigns) do
    ~H"""
    <div class="m-2 card w-full md:w-[45vw] bg-secondary/5 shadow-lg shadow-secondary/10 border border-secondary/5">
      <div class="card-body">
        <%!-- name --%>
        <div>
          <%= if @data.user do %>
            <%= @data.user.display_name %> (username: <b><%= @data.user.username %></b>)
        ...
        <%= if Enum.member?([:in_progress, :completed], @data.quiz_state) do %>
          ...
          <div class="mt-2">
            ...
            - <%= @data.score %> / <%= @data.current_card_index %> correct (<%= @data.quiz_length -
              @data.current_card_index %> remaining)
          </div>
          <div class="show-empty-element">
            <div if={@data.current_card_index > 0}></div>
          </div>
        <% end %>
      </div>
    </div>
